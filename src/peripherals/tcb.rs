...
impl MemoryMapped for Tcb {
    ...
    fn read(&mut self, address: usize) -> (u8, usize) {
        match address {
            ...
            TCB_CCMPL => {
                self.regs[TCB_TEMP] = self.regs[TCB_CCMPH];
                (self.regs[TCB_CCMPL], 0)
            }
            TCB_CCMPH => (self.regs[TCB_TEMP], 0),
            TCB_CNTL => {
                self.regs[TCB_TEMP] = self.regs[TCB_CNTH];
        ...
        match address {
            ...
            TCB_CTRLB => {
                ...
                self.cntmode = match value & 0x07 {
                    ...
                    0x01 => {
                        println!(
                            "[WARNING] TIMEOUT mode is not implemented for TCB in this emulator."
                        );
                        TCB_MODE::TIMEOUT
                    }
                    0x02 => {
                        println!(
                            "[WARNING] CAPT mode is not implemented for TCB in this emulator."
                        );
                        TCB_MODE::CAPT
                    }
                    0x03 => {
                    ...
                    0x05 => {
                        println!(
                            "[WARNING] FRQPW mode is not implemented for TCB in this emulator."
                        );
                        TCB_MODE::FRQPW
                    }
                    0x06 => {
                        println!(
                            "[WARNING] SINGLE mode is not implemented for TCB in this emulator."
                        );
                        TCB_MODE::SINGLE
                    }
                    0x07 => {
                        println!(
                            "[WARNING] PWM8 mode is not implemented for TCB in this emulator."
                        );
                        TCB_MODE::PWM8
                    }
                    _ => TCB_MODE::PWM8,
            ...
            TCB_CCMPH => {
                self.regs[TCB_CCMPH] = value;
                self.regs[TCB_CCMPL] = self.regs[TCB_TEMP]
            }
            TCB_CNTL => self.regs[TCB_TEMP] = value,
            TCB_CNTH => {
                self.regs[TCB_CNTH] = value;
                self.regs[TCB_CNTL] = self.regs[TCB_TEMP]
            }
            _ => {}
        }
        0
        ...
        if self.enabled {
            match self.clksel {
                ...
                TCB_CLKSEL::DIV2 => {
                    ...
                    if self.tictoc {
                        return;
                    }; // Only continue every second tick
                }
